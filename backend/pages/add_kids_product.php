...
// --- Default product_code so it's always defined ---
$product_code = 'KIDS' . time();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($alerts)) {
        $catStmt = mysqli_prepare($conn, "SELECT name FROM kids WHERE id = ?");
...
<div class="main-content app-content" style="background-color:rgb(42, 42, 44); min-height:100vh; color:#fff;">
  <div class="container-fluid py-4">
    <h2 class="mb-4 fw-bold">Add Kid's Product</h2>
...
<script>
  ...
  document.getElementById('add-variation').addEventListener('click',()=>{
    ...
    row.innerHTML=`
      <div class="col-md-4"><input type="text" name="attribute[]" class="form-control" placeholder="Variation (e.g., S, M, L)" required></div>
      <div class="col-md-3"><input type="number" name="inventory_count[]" class="form-control" placeholder="Inventory" required></div>
      <div class="col-md-3"><input type="number" step="0.01" name="weight[]" class="form-control" placeholder="Weight (kg)" required></div>
      <div class="col-md-2 d-grid"><button type="button" class="btn btn-danger remove-row">&times;</button></div>`;

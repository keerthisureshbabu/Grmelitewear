...
$countRes = mysqli_query($conn, "SELECT COUNT(DISTINCT product_code) AS total FROM womens_products");
...
$productCodes = [];
$codesRes = mysqli_query($conn, "SELECT DISTINCT product_code FROM womens_products ORDER BY product_code LIMIT $limit OFFSET $offset");
...
if(isset($_GET['code'])){
    ...
    mysqli_query($conn, "DELETE FROM womens_products WHERE product_code='$code'");
    header("Location: view_womens_products.php?deleted=1");
    exit;
}
include("header.php");
...
if(!empty($productCodes)){
   ...
   $sql = "
            ...
            LEFT JOIN women k ON p.variation_id = k.id
...
<div class="container-fluid py-4">
  <h2 class="mb-4 fw-bold">Women's</h2>
    ...
    <?php foreach($products as $code => $prod): ?>
      <div class="card mb-4" style="background-color:#1e1e1e; border:1px solid #333; border-radius:30px;">
        <div class="card-header d-flex justify-content-between align-items-center" style="background-color:#0d6efd;">
          ...
          <div>
           <a href="update_womens_product.php?code=<?= urlencode($code); ?>" class="btn btn-sm" style="background-color:#ffc107; color:#000;">
                <i class="bi bi-pencil-square"></i>
            </a>

            <a href="view_womens_products.php?code=<?= urlencode($code); ?>" 
      ...
      <ul class="pagination">
        <?php for($i=1; $i<=$totalPages; $i++): ?>
          <li class="page-item <?= $i == $page ? 'active' : ''; ?>">
            <a class="page-link" href="view_womens_products.php?page=<?= $i; ?>" 

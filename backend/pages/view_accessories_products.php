<?php
// view_accessories_products.php (primary + secondary images)
include 'db.php';

// determine total distinct product_code from master if exists, otherwise from accessories_products
$totalProducts = 0;
$checkMaster = mysqli_query($conn, "SHOW TABLES LIKE 'accessories_products_master'");
if ($checkMaster && mysqli_num_rows($checkMaster) > 0) {
    $res = mysqli_query($conn, "SELECT COUNT(DISTINCT product_code) AS total FROM accessories_products_master");
} else {
    $res = mysqli_query($conn, "SELECT COUNT(DISTINCT product_code) AS total FROM accessories_products");
}
if ($res) {
...
if (!empty($_GET['code'])) {
    ...
    // Delete variants
    $delVariants = mysqli_prepare($conn, "DELETE FROM accessories_products WHERE product_code = ?");
    ...
    $checkMaster2 = mysqli_query($conn, "SHOW TABLES LIKE 'accessories_products_master'");
    if ($checkMaster2 && mysqli_num_rows($checkMaster2) > 0) {
        $delMaster = mysqli_prepare($conn, "DELETE FROM accessories_products_master WHERE product_code = ?");
    ...
    header("Location: view_accessories_products.php?deleted=1");
    exit;
}

// fetch product codes page-wise (distinct product_code)
$productCodes = [];
$codesSql = "SELECT DISTINCT product_code FROM accessories_products ORDER BY product_code LIMIT ? OFFSET ?";
...
if (!empty($productCodes)) {
    ...
    $sql = "
        ...
        LEFT JOIN accessories k ON p.variation_id = k.id
        LEFT JOIN accessories_products_master m 
    ...
    if ($result) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            if (!isset($products[$pcode])) {
                ...
                // if all master images are empty, fall back to first variant images
                if (implode('', $primaryImages) === '') {
                    $primaryImages = [
                        $row['v_image1'] ?? '',
                        $row['v_image2'] ?? '',
                        $row['v_image3'] ?? '',
                ...
                $products[$pcode] = [
                    ...
                    'primary_images' => $primaryImages, // always 4 entries
                    'mrp_price'      => (float)($row['mrp_price'] ?? 0),
                    'actual_price'   => (float)($row['actual_price'] ?? 0),
                    'variations'     => []
                ];
            }
...
<div class="main-content app-content" style="background-color:rgb(42, 42, 44); min-height:100vh; color:#fff;">
  <div class="container-fluid py-4">
    <h2 class="mb-4 fw-bold">Accessories</h2>
        ...
        <div class="card mb-4" style="background-color:#1e1e1e; border:1px solid #333; border-radius:12px;">
          <div class="card-header d-flex justify-content-between align-items-center" style="background-color:#0d6efd; border-radius:12px 12px 0 0;">
            <div>
              ...
              <span class="badge ms-2"
                ...
                <?= htmlspecialchars($prod['variation_name']); ?>
              </span>
            </div>
            <div>
              <a href="update_accessories_products.php?product_code=<?= urlencode($code); ?>" class="btn btn-sm" style="background-color:#ffc107; color:#000;">
                <i class="bi bi-pencil-square"></i>
              </a>

              <a href="view_accessories_products.php?code=<?= urlencode($code); ?>"
                 class="btn btn-sm"
          ...
          <div class="card-body">
            ...
            <div class="mb-3" style="color:#ffc107;">
              <strong>MRP:</strong> ₹<?= number_format((float)$prod['mrp_price'], 2); ?> &nbsp; |
              &nbsp; <strong>Actual Price:</strong> ₹<?= number_format((float)$prod['actual_price'], 2); ?>
            </div>
                ...
                <tbody>
                  <?php foreach ($prod['variations'] as $v): ?>
                    <tr style="transition: background-color 0.3s;"
                      ...
                      <td style="border-top:1px solid #333; padding:12px;"><?= htmlspecialchars((string)$v['inventory_count']); ?></td>
        ...
        <ul class="pagination">
          <?php for ($i = 1; $i <= $totalPages; $i++): ?>
            <li class="page-item <?= $i == $page ? 'active' : ''; ?>">
              <a class="page-link" href="view_accessories_products.php?page=<?= $i; ?>"

...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $variation_id = (int)($_POST['variation_id'] ?? 0); // category id from accessories table
    ...
    // fetch category name
    if (empty($alerts)) {
        $catStmt = mysqli_prepare($conn, "SELECT name FROM accessories WHERE id = ?");
    ...
    if (empty($alerts)) {
        ...
        $createMaster = "CREATE TABLE IF NOT EXISTS accessories_products_master (
        ...
        $mSql = "INSERT INTO accessories_products_master (product_code, product_name, variation_id, variation_name, mrp_price, actual_price, primary_image1, primary_image2, primary_image3, primary_image4) VALUES (?,?,?,?,?,?,?,?,?,?)";
        ...
        if (!$mStmt) {
            ...
            if (!mysqli_stmt_execute($mStmt)) {
                ...
                if (stripos($err, 'Duplicate') !== false || stripos($err, 'duplicate') !== false) {
                    ...
                    $uSql = "UPDATE accessories_products_master SET product_name=?, variation_id=?, variation_name=?, mrp_price=?, actual_price=?, primary_image1=?, primary_image2=?, primary_image3=?, primary_image4=? WHERE product_code=?";
    ...
    // process variants (multiple rows) — secondary images per color stored in accessories_products.image1..image4
    if (empty($alerts) || true) {
        ...
        if (!empty($_POST['variant_type']) && is_array($_POST['variant_type'])) {
            $sql = "INSERT INTO accessories_products 
...
<div class="main-content app-content" style="background-color:rgb(42, 42, 44); min-height:100vh; color:#fff;">
  <div class="container-fluid py-4">
    <h2 class="mb-4 fw-bold">Add Accessories's Product</h2>

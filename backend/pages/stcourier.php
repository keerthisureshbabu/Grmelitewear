...
if (isset($_POST['action']) && $_POST['action'] === 'add') {
    ...
    if ($tracking_no === '') {
        ...
        $stmt = $conn->prepare("INSERT INTO stcourier_numbers (tracking_no) VALUES (?)");
        if ($stmt) {
...
$totalRes = $conn->query("SELECT COUNT(*) AS total FROM stcourier_numbers");
$totalRow = $totalRes->fetch_assoc();
$totalRows = (int)$totalRow['total'];
$totalPages = (int)ceil($totalRows / $limit);

$result = $conn->query("SELECT * FROM stcourier_numbers ORDER BY id DESC LIMIT $limit OFFSET $offset");
?>
        ...
        <ul class="pagination">
          <?php for ($i = 1; $i <= $totalPages; $i++): ?>
            <li class="page-item <?= $i == $page ? 'active' : ''; ?>">
              <a class="page-link" href="stcourier.php?page=<?= $i; ?>"

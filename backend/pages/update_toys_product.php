...
function uploadIndexedFile($field, $idx, $upload_dir = "uploads/") {
    ...
    // Expect arrays: name[tmp], tmp_name[tmp]
...
// master
$masterStmt = mysqli_prepare($conn, "SELECT product_name, variation_id, variation_name, mrp_price, actual_price, primary_image1, primary_image2, primary_image3, primary_image4 FROM toys_products_master WHERE product_code = ?");
...
// variants
$vstmt = mysqli_prepare($conn, "SELECT id, variation_code, attribute, inventory_count, image1, image2, image3, image4, product_name, variation_name, mrp_price, actual_price FROM toys_products WHERE product_code = ? ORDER BY id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($alerts)) {
        ...
        try {
            ...
            $tv = mysqli_prepare($conn, "SELECT name FROM toy WHERE id = ?");
            if ($tv) {
            ...
            // upsert master
            $upsertSql = "INSERT INTO toys_products_master
            ...
            // wipe variants
            $del = mysqli_prepare($conn, "DELETE FROM toys_products WHERE product_code = ?");
            ...
            if (isset($_POST['variation_code']) && is_array($_POST['variation_code'])) {
                $ins = mysqli_prepare($conn, "INSERT INTO toys_products
            ...
            mysqli_commit($conn);
            header("Location: view_toys_products.php?updated=1");
            exit;
...
<div class="main-content app-content" style="background-color:rgb(42, 42, 44); min-height:100vh; color:#fff;">
  <div class="container-fluid py-4">
    <h4 class="mb-3"><b>Update Toy's</b></h4>
    ...
    <div class="card-body">
      <form method="post" enctype="multipart/form-data">
        ...
        <a href="view_toys_products.php" class="btn btn-secondary">Back</a>
      </form>
    </div>
  </div>
</div>
